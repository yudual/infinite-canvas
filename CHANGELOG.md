...
- [优化] 降低后台用户管理与调用审计的信息密度，新增用户详情抽屉及审计筛选摘要，保留高频操作入口。

- [优化] 精简后台管理页的状态提示与公告配置信息层级，增加未保存草稿提示并突出发布动作。

+ [修复] 管理后台概览在统计数据尚未返回时不再触发类型错误；上传素材写入数据库失败时自动清理已落盘文件，并在生产环境拒绝使用默认 JWT 密钥。

+ [修复] 修复前端生图工作台与画布下拉列表未同步服务端多渠道模型的问题，实现服务端渠道全生命周期自动同步与代理直通。
+ [新增] 管理后台重构为基于 Ant Design Token 的多渠道池管理、素材库运维、云端工程管理与调用审计日志面板。
+ [新增] 服务端新增 AI 调用审计日志系统（ai_audit_logs 与 AiAuditService），拦截记录生图/修图/对话全量请求明细、响应耗时、命中渠道与重试故障转移，实现 Zero Base64 Bleed 图像摘要与 API 密钥自动脱敏，并提供管理后台审计日志检索 API (/api/admin/audit-logs)。
+ [新增] 服务端新增多渠道 AI 模型服务池（ai_channels）与智能容灾路由（ai-router），支持优先级与权重调度、模型自动探活同步、429/5xx/超时自动故障转移（Failover）及零密钥泄露。
+ [新增] 新增轻量 Node.js/TypeScript + SQLite 后端服务、首次访问初始化向导 (/setup) 与基于 JWT 的多用户认证登录系统 (/login)。
+ [新增] 新增管理后台 (/admin) 及用户管理、AI 服务配置与系统运行概览功能。
+ [新增] 新增安全 AI 代理服务 (/api/ai/*)，支持生图、修图与 SSE 流式对话转发，服务端动态注入 API 凭据并隔离密钥防泄露。
+ [新增] 新增云端画布工程持久化 (/api/projects/*) 与素材文件本地存储及静态托管服务 (/api/assets/*, /uploads/*)。
+ [优化] 深度重构管理后台 AI 模型配置，支持主流服务商一键预设、上游模型自动发现归类、热门模型快捷添加与连通性延时诊断。
+ [修复] 修复生图代理中上游（如 chatgpt2api 等）返回跨域/易过期 CDN 图片 URL 时前端因 CORS 导致获取失败的问题，服务端自动下载转 Base64 缓存并提供全局图片代理。
+ [修复] 修复管理后台 AI 配置表单被清空与用户列表搜索翻页竞态重置问题，支持普通用户探测模型及修复以图生图代理。
