                                ...
                                >
                                    {references.map((item, index) => (
                                        <div key={item.id} className="group relative size-20 shrink-0 overflow-hidden rounded-md border border-stone-200 dark:border-stone-800">
                                            ...
                                            <button type="button" className="absolute right-1 top-1 flex sm:hidden sm:group-hover:flex size-6 items-center justify-center rounded bg-black/60 text-white transition" onClick={() => setReferences((value) => value.filter((ref) => ref.id !== item.id))} aria-label={t("videoWorkbench.removeImage")}>
